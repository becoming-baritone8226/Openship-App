...
/**
 * Thin MCP client wrapper: connect to `/api/mcp` with PAT auth, discover tools
 ...
 *
 * ponytail: in-memory catalog only — re-fetch on connect/foreground. No
 * persistent cache: a stale catalog would lie after token or permission changes.
 * Unavailable-MCP (older server, missing `/api/mcp`, read-only token) surfaces
 * as [connect] failure + [hasTool] returning false, so callers degrade to the
 * Phase 1 read-only experience without extra plumbing here.
 */
class McpClient(
